...
AstrBot 是一个开源的多平台聊天机器人框架，专注于将大语言模型（LLM）接入日常社交工具（如 QQ、微信、Telegram），提供对话管理、插件扩展和自动化任务能力。其核心定位是 **“消息平台与大模型的连接器”**，支持多轮对话、知识库、代码执行器等高级功能。
...
1️⃣ 安装 App  
2️⃣ 等待应用启动完成（第一次启动时，初始化过程将下载大量资源，耗时较久）
3️⃣ 扫码登录 QQ  
4️⃣ 填入大模型密钥  
...
8. **填写大模型 API 密钥**  
 - 前往“配置”页面，在“LLM 大模型”部分填入你的模型服务商提供的 Token（API Key）。  
